...
같은 문제를 여러 날짜에 다시 확인한 경우 하나의 항목으로 합치고 최초 제보일과 재확인 날짜를 함께 남겼습니다. 각 항목을 펼치면 문제 상황과 개선 방향을 볼 수 있습니다.

<details>
<summary><strong>날짜·시간 입력을 달력과 시계 중심으로 개선</strong> · 2026.07.22, 07.24, 08.01, 08.06</summary>

- **발견:** 직접 입력하거나 긴 선택 목록을 사용하는 방식은 날짜와 시간을 빠르게 고르기 어려웠습니다. 시계형 입력을 적용한 뒤에는 숫자가 바늘보다 한 칸씩 어긋나 선택값을 직관적으로 확인하기 어려웠고, 시간을 잘못 눌러도 처음 상태로 되돌릴 수 없었습니다.
- **개선:** 날짜 선택은 달력, 시간 선택은 시계형 입력으로 통일하고 시간은 1시간, 분은 5분 단위로 선택하도록 했습니다. 시계 숫자와 바늘의 위치를 같은 기준으로 맞추고, 잘못 선택한 시간을 기본값인 20분으로 되돌리는 초기화 버튼을 추가했습니다. 종일 일정과 반복 종료일 선택 흐름도 함께 지원합니다.
...
<details>
<summary><strong>잠긴 돌·돌 이름·외형의 식별성 개선</strong> · 2026.07.29, 08.01, 08.05</summary>

- **발견:** 잠긴 돌의 실루엣이 잘 보이지 않았고, 햇살돌·우유빛돌·토스트돌·복숭아돌처럼 이름과 외형의 인상이 맞지 않는 사례가 있었습니다. 8월 5일에는 우유빛돌과 딸기우유돌이 모두 흰 우유처럼 보여 이름이 다른데도 구분되지 않았습니다.
- **원인:** 딸기우유돌에는 분홍색 전용 표현이 준비되어 있었지만, 공통으로 들어간 짧은 이름 조각인 ‘우유돌’을 먼저 검사해 일반 우유빛돌 표현이 적용됐습니다.
- **개선:** ‘딸기우유’처럼 더 구체적인 이름을 먼저 판별하도록 순서를 고쳤습니다. 224종 전체에는 이름 의미에 맞춘 색상 계열과 8가지 세부 무늬·각도를 조합해 지나치게 비슷한 외형도 함께 줄였습니다.
- **검증:** 우유빛돌은 흰 우유, 딸기우유돌은 분홍 딸기우유로 분리되는지 대표값을 확인하고, 224종의 최종 외형 서명이 서로 겹치지 않는지 자동 테스트로 검사했습니다.
...
핵심 도메인 로직과 화면 회귀 조건을 Node.js 내장 테스트 러너로 검증합니다. 현재 **16개 테스트 파일, 70개 테스트 케이스**가 포함되어 있습니다.
